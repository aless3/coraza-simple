...
func processRequest(tx types.Transaction, req *http.Request) (*types.Interruption, error) {
	...
	// Use net.SplitHostPort instead of manually find the port looking for the separator `:`
	client, port, err := net.SplitHostPort(req.RemoteAddr)
	if err != nil {
		tx.DebugLogger().Error().Err(err).Msg("Failed to parse client and port from RemoteAddr")
	}

	// convert to int because net.SplitHostPort returns the port as a string, the rest of the original program expects an int
	if cport, err = strconv.Atoi(port); err != nil {
		tx.DebugLogger().Error().Err(err).Msg("Failed to parse port as integer from RemoteAddr")
