...
func setupTestServer(t *testing.T) *httptest.Server {
	t.Helper()
	waf := createWAF()
	return httptest.NewServer(txhttp.WrapHandler(waf, http.HandlerFunc(defaultHandler)))
}
...
func TestHttpServerMain(t *testing.T) {
	...
	tests := []struct {
		...
		{"positive for query parameter", "/?id=0", 403, map[string]string{
			"DIRECTIVES_FILE": "./testdata/deny-id-0.conf",
		}, nil},
